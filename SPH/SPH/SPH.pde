...
void normalize_mass()
{
   ...
   for (int i = 0; i < State.NumParticles; ++i) 
   {
      println("pho "+state.rho[i]);
      rho2s += (state.rho[i])*(state.rho[i]);
      rhos += state.rho[i];
   }
   state.mass *= ( rho0*rhos / rho2s );
   println("Normalized mass "+state.mass);
}
  ...
  for (int i = 0; i < State.NumParticles; ++i) 
  {
    ...
    for (int j = 0; j < obstacles.size(); j++)
    {
      ...
      //if (b)
      {
        ...
        state.accelerations[i].x += force.x;   //repel
        state.accelerations[i].y += force.y;       // repel 
      }
    }    
  }
  ...
  for (int i = 0; i < State.NumParticles; ++i) 
  {
    ...
    for (int j = i+1; j < State.NumParticles; ++j) 
    {
      ...
      if (r2 < h2) 
      {
        ...
        // viscosity: mu * sum (mass * dvx / rho) * 45/pi/h^6 * (h - r)
        // pressure: -sum ( mass * (pi+pj)/2/rho * 15/pi/h^6 * (h - r)^3 )
        //     where p = k(rho - rho0)
