...
class State
{
   constructor()
   {
      this.NumParticles = 100; // number of particles
      this.mass = 0; // mass
...
function init_fluid()
{
   var d = displayDensity();
  var numcols = 3;
  var margin = params.r * 2.2;
  for (var i = 0; i < state.NumParticles; i++)
  {
     var cellj = i % numcols;
     var celli = floor(i/numcols);

     var pos = createVector(params.r+celli*margin, params.r+cellj*margin);
     pos.x += fluidOffset.x;
     pos.y += fluidOffset.y;

     var idx= 4 * ((pos.y * d) * width * d + (pos.x * d));
     //var icolor = bg.get(pos.x, pos.y); //imgPixels[idx];
     var ii = Math.floor(p.x);
     var jj = Math.floor(p.y);
     var icolor = imgPixels[ii][jj];
     console.log(icolor);

     state.positions[i] = pos;
     state.velocities[i] = createVector(0,0);
     state.accelerations[i] = createVector(0,0);
     state.colors[i] = icolor;
...
function draw() 
{
  ...
  //noStroke();
  var d = bg.displayDensity();
  for (var i = 0; i < state.NumParticles; i++)
  {
    var p = state.positions[i];
    var idx = 4 * ((p.y * d) * width * d + (p.x * d)); 
    var r = img.pixels[0]; //(p.x, p.y);
    var g = 0; //img.get(p.x, p.y);
    var b = 0; //img.get(p.x, p.y);
    var a = state.rho[i]/state.maxdensity;
    var c = color(r, g, b, a);
    fill(c);
